...
from SAT_model_rotations_opt_linear import *
...
def main(rotation: bool = False):
    ...
    for instance_file in glob.glob(pt.join(DEFAULT_INSTANCES_DIR, '*')):
        ...
        for var in map(str, variables):
            ...
            if len(instance_data['circuits'][k % instance_data['n']]) == 2:
                if k in range(instance_data['n']):
                    instance_data['circuits'][k] = (instance_data['circuits'][k]
                                                    + [x, y])
                else:
                    instance_data['circuits'][k % instance_data['n']] = (instance_data['circuits'][k % instance_data['n']][::-1]
                                                    + [x, y])
